...
var pre_carga = url.substring(37, 46);
...
console.log("pre_carga----", pre_carga);
$("#banco_label").hide();
...
////////////////////////////////////////////////////////////////////////
		      	          ...
		      	          function success(data, status, req){
		      	        	  ...
		      	        	  if(data1.return.descripcion === "OK"){
			      	        		...
			      	        		function processSuccess(data, status, req){
			      	        			...
			      	        			console.log("correo del comercio al consultarlo ", data.return.email);
					 		    	   
					 		    	  $.ajax({
								           type: "POST",
								           url: '/CBPult/Afiliacion/enviarCorreo/'+data.return.email+'',
								           dataType: "json",
								           success: processSuccess,
								           error: processError
								       });
	...
	if(pre_carga === "pre_carga"){
		
		//Cargar Datos de la Pre-Carga//
		
		$.ajax({
            type: "GET",
            url: '/CBPult/Afiliacion/consultaPrecargaCOmercioPorIdentificacionComercio/'+id+'',
            dataType: "json",
            success: processSuccess,
            error: processError
		});
		...
		function processSuccess(data, status, req) {
            //alert(req.responseText + " " + status);
       		...
       		$('select[id="tipo_identificacion_tab2"] option:selected').val(data.return.tipoIdentificacionId.nombre);
       		$("#identificacion_tab2").val(data.return.identificacionComercio);
       		$("#nombre_empresa_tab2").val(data.return.nombreEmpresarial);
       		$('select[id="estado_tab5"] option:selected').val(data.return.idDistrito.cantonId.provinciaId.nombreProvincia);
       		$('select[id="municipio_tab5"] option:selected').val(data.return.idDistrito.cantonId.nombreCanton);
       		$('select[id="ciudad_tab5"] option:selected').val(data.return.idDistrito.nombreDistrito);
           	$("#sector_urbanizacion_tab5").val(data.return.urbanizacion);
           	$("#geo_localizacion_tab5").val(data.return.geolocalizacion);
           	$("#punto_referencia_tab5").val(data.return.puntoReferencia);
           	
           	$('select[id="tipo_identificacion_tab6"] option:selected').val(data.return.tipoIdentifContactoId.nombre);
           	$("#identificacion_tab6").val(data.return.identificacionContacto);
           	$("#telefono_tab6").val(data.return.telefonoCelular);
           	$("#primer_nombre_tab6").val(data.return.primerNombre);
           	$("#segundo_nombre_tab6").val(data.return.segundoNombre);
           	$("#primer_apellido_tab6").val(data.return.primerApellido);
           	$("#segundo_apellido_tab6").val(data.return.segundoApellido);
		              	...
		              	console.log("form----", $('#divFiles')[0]);
		      	          ...
		      	          function success(data, status, req){
		      	        	  ...
		      	        	  if(data1.return.descripcion === "OK"){
		      	        		  	...
		      	        		  	$.ajax({
				 		               type: "POST",
				 		               url: '/CBPult/Afiliacion/actualizaStatusPrecargaComercio',
				 		               contentType: "application/json",
				 		               dataType: "json",
				 		               data: JSON.stringify(actualiza_status_precarga),
				 		               success: processSuccess,
				 		               error: processError
				        	 		});
						 		       ...
						 		       function processSuccess(data, status, req){
						 		    	  ...
						 		    	  $.ajax({
									           type: "POST",
									           url: '/CBPult/Afiliacion/enviarCorreo/'+correo+'',
									           dataType: "json",
									           success: processSuccess,
									           error: processError
									       });
